<template>
  <section id="proyectos" class="contenedor-proyectos-galeria">
    ...
    <div class="galeria-carrusel">
      <div class="proyectos-wrapper" ref="carruselRef" @scroll="checkScroll">
        <div
        ...
        >
          <div class="tarjeta-flip-inner">
            <div class="tarjeta-flip-front">
              <div class="imagen-contenedor-visual">
                ...
                <div class="imagen-overlay">
                  <span class="neon-text-pulse">{{
                    proyecto.demo ? "DEMO ONLINE" : "En Construcción"
                  }}</span>
                </div>
              </div>
            ...
            <div class="tarjeta-flip-back">
              ...
              <p class="descripcion-extendida">
                {{ proyecto.descripcionExtendida || proyecto.descripcion }}
              </p>
...
const proyectos = ref([
  {
    ...
    descripcion:
      "Plataforma de comercio electrónico con carrito de compras y gestión de inventario.",
    descripcionExtendida:
      "Un robusto e-commerce construido con Vue.js para el frontend, Node.js y Express para la API backend, y MongoDB como base de datos. Incluye autenticación de usuarios, pasarela de pago simulada, gestión de productos y carrito de compras dinámico. Optimizado para una experiencia de usuario fluida y segura.",
  ...
  },
  {
    ...
    descripcion:
      "Creación de un diseño de portafolio minimalista con fuerte estética Cyberpunk y efectos de luz neón.",
    descripcionExtendida:
      "Un diseño de portafolio web con una marcada estética Cyberpunk. Implementado con Vue 3 para componentes dinámicos y SCSS para estilos modulares. Incorpora animaciones CSS complejas para efectos de neón, paralaje y transiciones fluidas que capturan la esencia futurista.",
  ...
  },
  {
    ...
    tecnologias: ["Java"],
    descripcion:
      "Un pequeño juego de ahorcado cdidactico con intergfaz GUI y lógica de juego básica en java.",
    descripcionExtendida:
      "Desarrollo de un juego de ahorcado en 2D . Material educativo.EN DESARROLLO",
...
<style scoped>
/*----- VARIABLES -----*/
/* Asegúrate de que estas variables estén definidas en un archivo CSS global (ej. base.css) 
o en el componente App.vue para que var(--vt-c-*) funcionen. */

/*----- ANIMACIONES COMPARTIDAS (Aseguramos uso de variables globales) -----*/

@keyframes star-fly-proy {
  0% {
    opacity: 1;
...
@keyframes pulse-neon-text {
  ...
  50% {
    /* Usando variable global */
    text-shadow: 0 0 15px var(--vt-c-magenta-pure), 0 0 25px var(--vt-c-magenta-pure);
    opacity: 1;
  }
}
@keyframes move {
...
.titulo-seccion {
  ...
  font-size: 2.8rem;
  font-weight: 700;
  /* CORREGIDO: Usando variable global */
  color: var(--vt-c-white);
  text-shadow: 
    /* CORREGIDO: Usando variable global */ 0 0 10px var(--vt-c-white),
    0 0 25px var(--vt-c-white),
    /* Manteniendo el color RGB para el tercer efecto de sombra neón */ 0 0 40px
      rgba(57, 255, 20, 0.8);
  padding-bottom: 0.5rem;
  /* Manteniendo el color RGB para el borde inferior neón */
...
.proyectos-wrapper {
  ...
  /* CORREGIDO: Usando variables globales */
...
.proyectos-wrapper::-webkit-scrollbar-track {
  background: var(--vt-c-black-soft);
  border-radius: 10px;
}
/* CORREGIDO: Usando variable global */
.proyectos-wrapper::-webkit-scrollbar-thumb {
  background: var(--vt-c-indigo);
  border-radius: 10px;
}

/* ------------------------------------------------------------- */
/* TARJETA FLIP - CONTENEDOR PRINCIPAL */
/* ------------------------------------------------------------- */
.tarjeta-flip-container {
  --accent-color: #a855f7; /* Esto se sobreescribe en el :style */
  flex-shrink: 0;
  width: 300px;
  height: 400px;
  perspective: 1000px;
...
.tag-tech {
  ...
  /* CORREGIDO: Usando variable global RGBA para el fondo. Asumiendo que definiste --vt-c-indigo-rgba-15 */
  background-color: var(--vt-c-indigo-rgba-15);
  /* CORREGIDO: Usando variable global para el color del texto */
  color: var(--color-neon-cyan);
  border-radius: 4px;
  padding: 2px 6px;
  margin-left: 5px;
  font-weight: 400;
  /* CORREGIDO: Usando variable global RGBA para la sombra, si no existe usa el color sólido */
  text-shadow: 0 0 3px var(--color-neon-cyan-rgba-50);
  font-size: 0.7rem;
}
...
.imagen-miniatura-proyecto {
  ...
  opacity: 1;
  filter: saturate(1); /* Saturación normal */
  filter: contrast(1); /* Contraste normal */
...
.imagen-overlay {
  ...
  width: 100%;
  height: 100%;
  /* CORREGIDO: Usando variable global RGBA. Asumiendo que definiste --vt-c-dark-blue-rgba-30 */
  background: var(--vt-c-dark-blue-rgba-30);
  display: flex;
  justify-content: center;
...
.boton-accion-enlace {
  --clr: var(--clr);
  /* CORREGIDO: Usando variable global para el fondo interno */
  --inner-bg: var(--vt-c-dark-blue);
  position: relative;
  display: inline-block;
  ...
  overflow: hidden;
  transition: 0.5s;
  /* Usando la variable interna corregida */
...
.boton-accion-enlace span {
  position: relative;
  z-index: 1;
}
.boton-accion-enlace:hover {
  /* CORREGIDO: Usando variable global para el color de texto */
  color: var(--vt-c-black-soft);
  background: var(--clr);
  box-shadow: 0 0 10px var(--clr), 0 0 25px var(--clr);
}
.boton-accion-enlace i {
  position: absolute;
  display: block;
}
.boton-accion-enlace i:nth-child(1) {
  top: 0;
  left: 0;
  width: 100%;
  height: 2px;
...
.boton-accion-enlace i:nth-child(2) {
  top: -100%;
  right: 0;
  width: 2px;
  height: 100%;
...
.boton-accion-enlace i:nth-child(3) {
  bottom: 0;
  right: 0;
  width: 100%;
  height: 2px;
...
.boton-accion-enlace i:nth-child(4) {
  bottom: -100%;
  left: 0;
  width: 2px;
  height: 100%;
